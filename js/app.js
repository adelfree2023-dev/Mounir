    ...
    generateBulk: async function (count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Enterprise E-Commerce Calculations
                ...
                const cities = citiesByCountry[country] || ['Capital City', 'Main City', 'Central City'];
                record['city'] = cities[Math.floor(Math.random() * cities.length)];

                // Select brand based on category
                if (record['productCategory'] && this.currentSchema.config.brandsByCategory) {
                    const brands = this.currentSchema.config.brandsByCategory[record['productCategory']];
                    if (brands && brands.length > 0) {
                        record['brand'] = brands[Math.floor(Math.random() * brands.length)];
                    }
                }

                // Calculate revenue based on quantity and price
                const quantity = parseFloat(record['quantity']) || 1;
                const unitPrice = parseFloat(record['unitPrice']) || 50;
                const discountPercent = parseFloat(record['discountPercent']) || 0;
                const cost = parseFloat(record['cost']) || (unitPrice * 0.6); // 60% cost margin
                ...
                record['grossRevenue'] = grossRevenue.toFixed(2);
                record['discountAmount'] = discountAmount.toFixed(2);
                record['netRevenue'] = netRevenue.toFixed(2);
                record['profit'] = profit.toFixed(2);

                // Calculate Customer Lifetime Value based on subscription
                if (record['subscriptionPlan']) {
                    const planFees = this.currentSchema.config.subscriptionFees;
                    const monthlyFee = planFees[record['subscriptionPlan']] || 0;

                    // Estimate months based on status
                    let estimatedMonths;
                    if (record['subscriptionStatus'] === 'Active') {
                        estimatedMonths = Math.floor(Math.random() * 24) + 6; // 6-30 months
                    } else {
                        estimatedMonths = Math.floor(Math.random() * 12) + 1; // 1-12 months (churned)
                    }

                    const lifetimeValue = monthlyFee * estimatedMonths;
                    record['lifetimeValue'] = lifetimeValue.toFixed(2);
                }
            }
