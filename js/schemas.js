const AppSchemas = {
    sales: {
        ...
        fields: [
            ...
            { id: 'orderDate', label: 'تاريخ الطلب', formLabel: 'تاريخ الطلب (من)', type: 'date', required: true },
            { id: 'orderEndDate', label: 'إلى', formLabel: 'إلى', type: 'date', uiOnly: true },
    ...
    purchasing: {
        ...
        fields: [
            ...
            { id: 'orderDate', label: 'تاريخ الطلب', formLabel: 'تاريخ الطلب (من)', type: 'date' },
            { id: 'orderEndDate', label: 'إلى', formLabel: 'إلى', type: 'date', uiOnly: true },
    ...
    enterprise_ecommerce: {
        ...
        title: 'Enterprise Subscription E-Commerce - EU Market',
        desc: 'منصة تجارة إلكترونية قائمة على الاشتراكات - جميع دول الاتحاد الأوروبي - بالدولار USD',
        fields: [
            // Customer Data
            { id: 'customerId', label: 'Customer ID', type: 'text', readonly: true },
            { id: 'customerName', label: 'Customer Name', type: 'text', required: true },
            { id: 'gender', label: 'Gender', type: 'select', options: ['Male', 'Female', 'Other'] },
            { id: 'ageGroup', label: 'Age Group', type: 'select', options: ['18-25', '26-35', '36-45', '46-55', '56-65', '66+'] },
            { id: 'country', label: 'EU Country', type: 'select', options: [
                'Germany', 'France', 'Italy', 'Spain', 'Poland', 'Netherlands', 'Belgium', 
                'Greece', 'Portugal', 'Czech Republic', 'Sweden', 'Hungary', 'Austria', 
                'Romania', 'Denmark', 'Finland', 'Slovakia', 'Ireland', 'Croatia', 
                'Lithuania', 'Slovenia', 'Latvia', 'Estonia', 'Cyprus', 'Luxembourg', 
                'Malta', 'Bulgaria'
            ], onChange: 'updateCities' },
            { id: 'city', label: 'City', type: 'text' },
            { id: 'customerSegment', label: 'Segment', type: 'select', options: ['VIP', 'Regular'] },
            { id: 'acquisitionChannel', label: 'Acquisition Channel', type: 'select', options: ['Organic Search', 'Paid Ads', 'Social Media', 'Referral', 'Email'] },
            
            // Sales Data
            { id: 'salesId', label: 'Sales ID', type: 'text', readonly: true },
            { id: 'orderDate', label: 'Order Date', formLabel: 'Order Date (From)', type: 'date', required: true },
            { id: 'orderEndDate', label: 'To', formLabel: 'To', type: 'date', uiOnly: true },
            ...
            { id: 'quantity', label: 'Quantity', type: 'number', required: true, onChange: 'calculateRevenue' },
            { id: 'unitPrice', label: 'Unit Price (USD)', type: 'number', required: true, onChange: 'calculateRevenue' },
            { id: 'grossRevenue', label: 'Gross Revenue (USD)', type: 'money', readonly: true },
            { id: 'discountPercent', label: 'Discount %', type: 'select', options: [0, 5, 10, 15, 20, 25, 30, 40], onChange: 'calculateRevenue' },
            { id: 'discountAmount', label: 'Discount Amount (USD)', type: 'money', readonly: true },
            { id: 'netRevenue', label: 'Net Revenue (USD)', type: 'money', readonly: true },
            { id: 'cost', label: 'Cost (USD)', type: 'number', onChange: 'calculateRevenue' },
            { id: 'profit', label: 'Profit (USD)', type: 'money', readonly: true },
            { id: 'salesChannel', label: 'Sales Channel', type: 'select', options: ['Website', 'Mobile App', 'Email Campaign'] },
            
            // Subscription Data
            { id: 'subscriptionId', label: 'Subscription ID', type: 'text', readonly: true },
            { id: 'subscriptionPlan', label: 'Subscription Plan', type: 'select', options: ['Basic ($9.99/mo)', 'Standard ($19.99/mo)', 'Premium ($39.99/mo)'] },
            { id: 'subscriptionStatus', label: 'Subscription Status', type: 'select', options: ['Active', 'Churned'] },
            { id: 'subscriptionStartDate', label: 'Subscription Start Date', type: 'date' },
            { id: 'churnReason', label: 'Churn Reason (if churned)', type: 'select', options: ['N/A', 'Too Expensive', 'Better Deal Elsewhere', 'Poor Customer Service', 'Slow Delivery', 'Switched to Competitor', 'Product Not Available', 'Technical Problems'] },
            { id: 'lifetimeValue', label: 'Customer Lifetime Value (USD)', type: 'money', readonly: true },
            
            // Support Data
            { id: 'supportTicketId', label: 'Support Ticket ID', type: 'text', readonly: true },
            { id: 'issueType', label: 'Issue Type', type: 'select', options: ['N/A', 'Product Defect', 'Delivery Issue', 'Billing', 'Technical Support', 'Returns'] },
            { id: 'resolutionTimeHours', label: 'Resolution Time (Hours)', type: 'number' },
            { id: 'csatScore', label: 'CSAT Score (1-5)', type: 'select', options: [1, 2, 3, 4, 5] }
        ],
        config: {
            brandsByCategory: {
                'Electronics': ['TechPro', 'PremiumChoice', 'ValueBrand'],
                'Fashion': ['StyleCo', 'PremiumChoice', 'ValueBrand'],
                'Home & Garden': ['HomeEssentials', 'EcoLife', 'ValueBrand'],
                'Sports': ['FitLife', 'PremiumChoice', 'ValueBrand'],
                'Beauty': ['BeautyMax', 'PremiumChoice', 'EcoLife'],
                'Books': ['BookWorld', 'ValueBrand'],
                'Toys': ['PlayTime', 'ValueBrand']
            },
            subscriptionFees: {
                'Basic ($9.99/mo)': 9.99,
                'Standard ($19.99/mo)': 19.99,
                'Premium ($39.99/mo)': 39.99
            }
        },
        handlers: {
            updateBrands: (val, form) => {
                const brands = AppSchemas.enterprise_ecommerce.config.brandsByCategory[val] || [];
                engine.populateSelect('brand', brands);
            },
            calculateRevenue: (form) => {
                const qty = parseFloat(form['quantity'].value) || 0;
                const price = parseFloat(form['unitPrice'].value) || 0;
                const discPct = parseFloat(form['discountPercent'].value) || 0;
                const cost = parseFloat(form['cost'].value) || 0;
                ...
                form['grossRevenue'].value = gross.toFixed(2);
                form['discountAmount'].value = discAmt.toFixed(2);
                form['netRevenue'].value = net.toFixed(2);
                form['profit'].value = profit.toFixed(2);
            }
        }
    }
